...
namespace Task.Connector.Tests
{
    public class UnitTest1
    {
        ...
        static string mssqlConnectionString = @"Server=localhost\SQLEXPRESS;Database=master;TrustServerCertificate=True;User Id=YourUsername;Password=Password1";
        static string postgreConnectionString = "Server=127.0.0.1;Port=5432;Database=postgres;Username=postgres;Password=postgres;";
        ...
        public IConnector GetConnector(string provider)
        {
            ...
            connector.Logger = new FileLogger($"{DateTime.Now.ToString("yyyy-dd-MM--HH-mm-ss")}connector{provider}.Log", $"{DateTime.Now}connector{provider}");
            return connector;
        }
