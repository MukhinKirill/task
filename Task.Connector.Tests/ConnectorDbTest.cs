...
{
    private static readonly string _requestRightGroupName = "Request";
    ...
    private static readonly Dictionary<string, string> _dataBasesCS = new()
    {
    ...
    public void CreateUser(string provider)
    {
        ...
        {
            Properties = new UserProperty[] { new("isLead", "false") }
        });
    ...
    public void GetUserProperties(string provider)
    {
        ...
        Assert.True(userInfo.FirstOrDefault(x => x.Value.Equals(DefaultData.MasterUser.TelephoneNumber)) != null);
    }
    ...
    public void UpdateUserProperties(string provider)
    {
        ...
        var propertyName = connector
            ...
            .First(x => x.Value.Equals(DefaultData.MasterUser.TelephoneNumber))
            .Name;
        var propsToUpdate = new UserProperty[] { new(propertyName,TestData.NewPhoneValueForMasterUser) };
    ...
    public void AddUserPermissions(string provider)
    {
        ...
        connector.AddUserPermissions(DefaultData.MasterUserLogin, new[] { RoleId });

        Assert.True(dataSetter.MasterUserHasITRole(dataSetter.GetITRoleId().ToString()));
        Assert.True(dataSetter
            .MasterUserHasRequestRight(dataSetter.GetRequestRightId(DefaultData.RequestRights[DefaultData.MasterUserRequestRights.First()].Name)
            .ToString()));
    }
    ...
    public void RemoveUserPermissions(string provider)
    {
        ...
        var requestRightIdToDrop = 
            $"{_requestRightGroupName}" +
            $"{_delimeter}" +
            $"{dataSetter.GetRequestRightId(DefaultData.RequestRights[DefaultData.MasterUserRequestRights.First()].Name)}";
        connector.RemoveUserPermissions(DefaultData.MasterUserLogin, new[] { requestRightIdToDrop });
